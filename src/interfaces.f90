module yaeos_interfaces
   ...
   use yaeos_autodiff, only: adiff => hyperdual
   ...
   abstract interface
      ...
      pure subroutine pures_property(z, v, t, property)
         ...
         import adiff
         type(adiff), intent(in) :: z(:), v, t
         type(adiff), intent(out) :: property(size(z))
      ...
      pure subroutine abs_cubic_mix(z, v, t, a, b, c, amix, bmix, cmix)
         ...
         import adiff
         type(adiff), intent(in) :: z(:), v, t, a(size(z)), b(size(z)), c(size(z))
         type(adiff), intent(out) :: amix, bmix, cmix
      end subroutine

      pure subroutine abs_cubic_prop_mix(z, v, t, prop, prop_mix)
         !-|  Subroutine to mix a single property
         import adiff
         type(adiff), intent(in) :: z(:), v, t, prop(size(z))
         type(adiff), intent(out) :: prop_mix
      end subroutine
