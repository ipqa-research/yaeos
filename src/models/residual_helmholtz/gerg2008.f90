...
!   RW status of diff variables: n:in t:in v:in arval:out
   ...
   &   arvalddd)
      ...
      temp = temp0
      temp0d = rho_rd*vd1
      temp0 = rho_r*vd + rho_rd*v
      temp2 = (SUM(nd)-temp*temp0)/(rho_r*v)
      temp1d = (-(temp0*tempd0)-temp*temp0d-temp2*rho_r*vd1)/(rho_r*v)
      temp1 = temp2
      temp2 = (-(temp0*tempd)-vd0*(rho_rd*temp+rho_r*temp1))/(rho_r*v)
      deltaddd = (-(tempd*temp0d)-temp0*tempdd-vd0*(rho_rd*tempd0+rho_r*&
      &     temp1d)-temp2*rho_r*vd1)/(rho_r*v)
      deltadd = temp2
      deltadd0 = temp1d
      deltad = temp1
      deltad0d = tempdd
      ...
      temp1 = temp2
      temp2 = (trd-temp1)/t
      temp0d = (-temp1d-temp2*td1)/t
      temp0 = temp2
      temp2 = (temp1/t-temp0)/t
      tauddd = td0*((temp1d-temp1*td1/t)/t-temp0d-temp2*td1)/t
      taudd = td0*temp2
      taudd0 = temp0d
      taud = temp0
      temp2 = tr*td0/(t*t)
      taud0d = temp2*2*td1/t
      taud0 = -temp2
      taud1 = -(tr*td1/t**2)
      tau = tr/t
      ar_puresd = 0.0_pr
...
!   RW status of diff variables: n:in t:in v:in arval:out
   ...
   &   , arvald, arvaldd)
      ...
      temp = temp0
      temp0 = rho_r*vd + rho_rd*v
      temp1 = (SUM(nd)-temp*temp0)/(rho_r*v)
      deltadd = (-(temp0*tempd)-(temp*rho_rd+temp1*rho_r)*vd0)/(rho_r*v)
      deltad = temp1
      deltad0 = tempd
      delta = temp
      temp1 = tr*td/t
...
!   RW status of diff variables: n:in t:in v:in arval:out
   ...
   &   tdb, arval, arvalb, arvald, arvaldb)
      ...
      CALL PUSHREAL8(temp)
      temp = SUM(n)/(v*rho_r)
      deltad = (SUM(nd)-temp*(rho_r*vd+v*rho_rd))/(v*rho_r)
      delta = temp
      taud = (trd-tr*td/t)/t
      tau = tr/t
      ar_puresd = 0.0_pr
      ...
      DO i=1,nc
         ad_from = i + 1
         DO j=ad_from,nc
            CALL PUSHINTEGER4(binary%i)
            CALL PUSHINTEGER4(binary%j)
            ...
            ! CALL PUSHPOINTER8(binary%nij)
            ! CALL PUSHPOINTER8(binary%dij)
            ! CALL PUSHPOINTER8(binary%tij)
            ! CALL PUSHPOINTER8(binary%ethaij)
            ! CALL PUSHPOINTER8(binary%epsij)
            ! CALL PUSHPOINTER8(binary%betaij)
            ! CALL PUSHPOINTER8(binary%gammaij)
      ...
      DO i=nc,1,-1
         ...
         DO j=nc,ad_from,-1
            ...
            ! CALL POPPOINTER8(binary%gammaij)
            ! CALL POPPOINTER8(binary%betaij)
            ! CALL POPPOINTER8(binary%epsij)
            ! CALL POPPOINTER8(binary%ethaij)
            ! CALL POPPOINTER8(binary%tij)
            ! CALL POPPOINTER8(binary%dij)
            ! CALL POPPOINTER8(binary%nij)
            ...
            CALL POPINTEGER4(binary%j)
            CALL POPINTEGER4(binary%i)
         END DO
      END DO
      ar_puresb = 0.0_pr
      ...
      trdb = tempb4
      tdb = tdb + tr*tempb3
      temp1 = rho_r*vd + v*rho_rd
      tempb3 = deltadb/(v*rho_r)
      tempb = deltab - temp1*tempb3
      ndb = ndb + tempb3
      tempb2 = -(temp*tempb3)
      tempb4 = -((SUM(nd)-temp*temp1)*tempb3/(v*rho_r))
      vb = vb + rho_r*tempb4 + rho_rd*tempb2
      rho_rb = v*tempb4 + vd*tempb2
      vdb = vdb + rho_r*tempb2
      rho_rdb = v*tempb2
      CALL POPREAL8(temp)
      tempb2 = tempb/(v*rho_r)
      nb = nb + tempb2
      tempb3 = -(SUM(n)*tempb2/(v*rho_r))
      vb = vb + rho_r*tempb3
      rho_rb = rho_rb + v*tempb3
      tempb2 = -(rho_rdb/vr**2)
...
!   RW status of diff variables: n:out t:out v:out arval:in-zero
   SUBROUTINE AR_B(model, n, nb, v, vb, t, tb, arval, arvalb)
      ...
      DO i=nc,1,-1
         CALL POPINTEGER4(ad_from)
         DO j=nc,ad_from,-1
            tempb0 = fij(i, j)*arvalb
            xb(i) = xb(i) + aij*x(j)*tempb0
            aijb = x(i)*x(j)*tempb0
            xb(j) = xb(j) + x(i)*aij*tempb0
            CALL POPREAL8(aij)
      ...
      trb = taub/t
      tb = tb - tr*taub/t**2
      tempb = deltab/(v*rho_r)
      nb = nb + tempb
      tempb0 = -(SUM(n)*tempb/(v*rho_r))
      vb = rho_r*tempb0
      rho_rb = v*tempb0
      vrb = -(rho_rb/vr**2)
