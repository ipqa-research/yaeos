...
contains
   ...
   subroutine GenericCubic_Ar(&
      ...
      ! Flag: true when D depends on V (e.g. s-DDLC mixing rule).
      ! Checked once and reused in every correction block below.
      logical :: volume_dep_D
      ...
      if (present(ArVn)) then
         ...
         ! s-DDLC correction: ArVn += (-fB)*dBi*dDdV + (-f)*dDidV
         if (volume_dep_D) ArVn(:) = ArVn(:) - fB*dBi(:)*dDdV - f*dDidV(:)
      end if
      ...
      if (present(ArTV)) then
         ...
         if (volume_dep_D) ArTV = ArTV + dDdV*f/T - f*dDdTV
      end if
