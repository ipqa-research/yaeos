program main
   use yaeos
   use yaeos__equilibria_multiphase_flash, only: pt_F_NP, solve_mp_flash_point, pt_mp_flash, MPEquilibriumState
   ...
   logical :: less_phases
   real(pr) :: S1, S2, tpd
   real(pr) :: x_l(np, nc), w(nc), K(np, nc)
   real(pr) :: betas(np), beta_w, P, T

   real(pr) :: F(psize), dX(psize)
   ...
   real(pr) :: ts(nt, npp), ps(nt, npp), nps(nt, npp)
   ...
   call assert(allclose(mpfr%x_l(1, :), fr%x, rtol=0.01_pr), "Heavy phase composition compared with SS flash")
   call assert(allclose(mpfr%w, fr%y, rtol=0.01_pr), "Light phase composition compared with SS flash")
   ...
   x_l(1, :) = [0.5507E-03, 0.6366E+00, 0.3613E+00, 0.1590E-02, 0.7322E-05, 0.4985E-07, 0.2840E-10]
   x_l(2, :) = [0.9999E+00, 0.6820E-04, 0.3450E-07, 0.1678E-29, 0.2427E-39, 0.2318E-55, 0.6331E-101]
   w = [0.3328E-03, 0.4025E+00, 0.8987E-01, 0.3861E+00, 0.5422E-01, 0.1674E-01, 0.5022E-01]

   call assert(allclose(mpfr%betas, [0.82526590969778513_pr, 1.4879261194292029E-003_pr, 0.17324616418278566_pr], rtol=0.01_pr), "Beta")
   call assert(allclose(mpfr%x_l(1, :), x_l(1, :), rtol=0.01_pr), "x_l1 phase composition")
   call assert(maxval(abs(mpfr%x_l(2, :)- x_l(2, :))) < 1e-4, "Water phase composition")
   ...
   !$OMP PARALLEL DO PRIVATE(i, j, T, P, mpfr) SHARED(model, z, dT, dP, T0, P0, ts, ps, nps)
   do i=1, npp
      P = P0 + i*dP
      do j=1, nt
         T = T0 + j*dT
...
contains
   ...
   subroutine numdiff
      ...
      do i=1,nc*np+np + 3
         ...
         call pt_F_NP(model, z, np, kinds_x, kind_w, Xdx, ns1, S1, ns2, S2, F1, tmp)
         XdX(i) = XdX(i) - 2*dx
         call pt_F_NP(model, z, np, kinds_x, kind_w, Xdx, ns1, S1, ns2, S2, F2, tmp)
         dfnum(:, i) = (F1 - F2)/(2*dX)
