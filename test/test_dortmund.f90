...
contains
   ...
   subroutine test_against_caleb_thermo()
      ...
      call assert(allclose(Gen2(1,:), &
         [-27.396545333109035_pr, 22.045031798841627_pr, -18.47735610933041_pr] / n_t, 1e-5_pr), "Gen2(1,:)" &
         )
      call assert(allclose(Gen2(2,:), &
         [22.04503179884173_pr, -22.7202581288098_pr, 23.170409015455117_pr] / n_t, 1e-5_pr), "Gen2(1,:)" &
         )
      call assert(allclose(Gen2(3,:), &
         [-18.47735610933015_pr, 23.17040901545502_pr, -26.299110952871235_pr] / n_t, 1e-5_pr), "Gen2(1,:)" &
         )
